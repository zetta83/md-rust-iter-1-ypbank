...
fn main() -> Result<(), ParserError> {
    ...
    let mut parser1 = ParserByType::from_format(
        &args.format1,
        fs::File::open(&args.file1)
            .map_err(|e| ParserError::IoError(format!("file1 not found: {}", e)))?,
    )?;
    let mut parser2 = ParserByType::from_format(
        &args.format2,
        fs::File::open(&args.file2)
            .map_err(|e| ParserError::IoError(format!("file2 not found: {}", e)))?,
    )?;
    ...
    loop {
        match (parser1.next(), parser2.next()) {
            ...
            (Some(Ok(_)), None) | (None, Some(Ok(_))) => {
                eprintln!("files different length");
                return Err(ParserError::DifferentLength);
            }
            (Some(Err(e)), _) | (_, Some(Err(e))) => return Err(e),
            (Some(Ok(r1)), Some(Ok(r2))) => {
                if r1 != r2 {
                    eprintln!("diff {}: {:?} vs {:?}", line_num + 1, r1, r2);
                    return Err(ParserError::DifferentRecord(line_num + 1));
                }
                line_num += 1;
            }
        }
